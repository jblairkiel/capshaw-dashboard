...
const DEFINITION = {
  id: 'visitor-follow-up', title: 'Request a follow-up', description: 'Ask a member to reach out to a guest.',
  fields: [
    { key: 'service', label: 'Service', type: 'select', required: true, options: [{ value: 'am', label: 'Sunday morning' }] },
    { key: 'notes', label: 'Notes', type: 'textarea' },
  ],
};

const INSTANCE_ROW = {
  id: 10, workflow: 'Visitor Follow-Up', title: 'Follow up with Sam Visitor', status: 'active',
...
const DETAIL = {
  success: true,
  instance: {
    workflow: 'Visitor Follow-Up', title: 'Follow up with Sam Visitor', status: 'active', stepId: 'elder-review',
    stepTitle: 'Awaiting elder approval', fields: [{ key: 'service', label: 'Service', value: 'Sunday morning' }],
...
describe('WorkflowPanel — visibility', () => {
  ...
  test('renders nothing on a page with no workflows and nothing in progress', async () => {
    ...
    const { container } = render(<WorkflowPanel page="visitors" user={MEMBER} />);
  ...
  test('shows the panel once a definition exists, even with nothing running', async () => {
    ...
    render(<WorkflowPanel page="visitors" user={MEMBER} />);
...
describe('WorkflowPanel — starting a workflow', () => {
  test('a single definition gets a button named after it, not a generic label', async () => {
    mockApi();
    render(<WorkflowPanel page="visitors" user={MEMBER} />);
    expect(await screen.findByRole('button', { name: 'Request a follow-up' })).toBeInTheDocument();
  });

  test('opens the start form with its fields, and a select with no options warns about it', async () => {
    mockApi();
    render(<WorkflowPanel page="visitors" user={MEMBER} />);
    fireEvent.click(await screen.findByRole('button', { name: 'Request a follow-up' }));

    expect(screen.getByText('Ask a member to reach out to a guest.')).toBeInTheDocument();
  ...
  test('warns when a select field has nothing to choose from', async () => {
    mockApi({ definitions: { success: true, definitions: [{
      ...DEFINITION, fields: [{ key: 'service', label: 'Service', type: 'select', options: [] }],
    }] } });
    render(<WorkflowPanel page="visitors" user={MEMBER} />);
    fireEvent.click(await screen.findByRole('button', { name: 'Request a follow-up' }));
  ...
  test('a refused start shows the error and stays on the form', async () => {
    mockApi({ start: { success: false, error: 'Missing required field: service' } });
    render(<WorkflowPanel page="visitors" user={MEMBER} />);
    fireEvent.click(await screen.findByRole('button', { name: 'Request a follow-up' }));
    fireEvent.click(screen.getByRole('button', { name: /Start workflow/i }));

    expect(await screen.findByText('Missing required field: service')).toBeInTheDocument();
    expect(screen.getByText('Ask a member to reach out to a guest.')).toBeInTheDocument();
  });
  ...
  test('starting can be cancelled', async () => {
    ...
    expect(screen.queryByText('Ask a member to reach out to a guest.')).not.toBeInTheDocument();
  });
});
...
describe('WorkflowPanel — the instance list', () => {
  ...
  test('the status and scope pickers refetch the list', async () => {
    ...
    render(<WorkflowPanel page="visitors" user={ADMIN} />);
...
describe('WorkflowPanel — opening the detail view', () => {
  test('clicking an instance shows its full detail', async () => {
    mockApi({ list: { success: true, instances: [INSTANCE_ROW] } });
    render(<WorkflowPanel page="visitors" user={MEMBER} />);
    fireEvent.click(await screen.findByText('Follow up with Sam Visitor'));
