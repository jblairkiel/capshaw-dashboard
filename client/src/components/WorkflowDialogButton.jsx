...
export default function WorkflowDialogButton({ page, user, label, title }) {
  ...
  return (
    <>
      ...
      {open && (
        <Dialog title={title} onClose={() => setOpen(false)} width="max-w-3xl">
          <WorkflowPanel page={page} user={user} title={title} embedded />
        </Dialog>
      )}
    </>
  );
